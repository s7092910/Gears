...
Returns every category as a `List<`[`IGlobalModSettingsCategory`](/docs/reference/global/i-global-mod-settings-category)`>`.
...
Returns every setting in every category of this tab, categories included, as a `List<`[`IGlobalModSetting`](/docs/reference/global/i-global-mod-setting)`>`.
