...
<MediaSlot
  title="/images/Preset-Selector.gif"
  alt="A Preset row stepping from Low to High, moving the three settings rows below it"
  ratio="aspect-4/3"
>
  The mod's settings page with the Preset row and the three rows it drives all visible at once. Step
  Preset from Low to Medium to High so each step visibly moves the rows below it. 3–5 seconds,
  looping, no cursor movement beyond the selector arrows.
</MediaSlot>
...
<MediaSlot
  title="/images/Preset-Custom.png"
  alt="The Preset row reading Custom after a driven setting was changed by hand"
  ratio="aspect-4/3"
>
  A still of the same category after the player has changed one driven row by hand, with the Preset
  row now reading Custom. Frame it exactly as the GIF above so the two read as before and after.
</MediaSlot>
...
<MediaSlot
  title="/images/Preset-Apply.png"
  alt="The Apply button lit after a preset moved several settings rows"
  ratio="aspect-4/3"
>
  The same page with the Apply button lit after a preset change, cropped so the moved rows and the
  button are visible together.
</MediaSlot>
