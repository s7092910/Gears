...
### Step 4: Turn `tooltipKey` into a `Description` element
...
| `OnSettingChangedEvent(IGlobalModSetting setting, string newValue)` | `OnSettingChangedEvent(IGlobalModSetting setting)`, with no value argument |
| — | `ValueChangedEvent<T>(IValueModSetting<T> setting, T newValue)` through `OnValueChanged`, global only |
| — | `ValueSelectedEvent<T>(IValueModSetting<T> setting, T newValue)` through `OnSelected` |
...
| — | `IModGlobalSettings.BindSettingsClass(Type)` and `IModWorldSettings.BindSettingsClass(Type)`, the `[Setting]` binding attribute, and the `[SettingOnValueChanged]`, `[SettingOnSelected]`, `[SettingOnChanged]` and `[SettingOnEnabled]` attributes |
...
<Tabs items={['GearsAPI 2.x', 'GearsAPI 3.0', 'GearsAPI 3.0, attribute form']}>
  ...
  <Tab value="GearsAPI 3.0, attribute form">
    This form needs no lookup at all:
...
- **Decide which value you want.** `CurrentValue` was the applied value, which is now
  ...
  `SelectedValue` now.
- **`OnSettingChanged` no longer carries the value.** Read `SettingValue` from the setting argument,
  or switch to `OnValueChanged`, which carries it, typed.
- **World settings have no `OnValueChanged` or `OnSettingChanged`.** Read `SettingValue` in
  `OnWorldSettingsLoaded`, which fires every time a world's values arrive.
