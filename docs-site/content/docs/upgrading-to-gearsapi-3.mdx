...
| — | `BindSettingsClass(Type)` and `SyncSettingsToClass(Type)` on both `IModGlobalSettings` and `IModWorldSettings`, the `[Setting]` binding attribute, and the `[SettingOnValueChanged]`, `[SettingOnSelectedChanged]`, `[SettingOnApplied]` and `[SettingOnEnabled]` attributes |
