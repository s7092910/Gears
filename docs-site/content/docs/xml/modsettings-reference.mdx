...
Gears drops a `Color` or `Binding` under `<World>` without a log line, and drops any other element
name the same way. When constructing a setting throws for any reason, Gears drops it with
