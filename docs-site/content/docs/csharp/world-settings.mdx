...
| `OnSelectedChanged` | yes | it fires while the host is picking values on the world settings screen |
...
[Change a setting from code](/docs/csharp/global-settings#change-a-setting-from-code).
...
Calling `BindSettingsClass` from it is still safe, and is the normal thing to do. Gears binds a given
type only the first time a settings tree sees it and does nothing on every call after that, so repeat
loads cannot pile up duplicate listeners. See [Binding the same type
twice](/docs/csharp/setting-listeners#binding-the-same-type-twice).

```csharp
public void OnWorldSettingsLoaded(IModWorldSettings worldSettings)
{
    // Safe to call on every load: only the first one binds.
    worldSettings.BindSettingsClass(typeof(MyWorldListeners));
}
```

Gears applies the incoming world values *before* it fires `OnWorldSettingsLoaded`, which is why the
two halves fit together: on the first load an
[`invokeOnBind`](/docs/csharp/setting-listeners#run-a-listener-once-at-bind-time) listener hands you
the values that just arrived, and on every load after that your already-subscribed
`OnSelectedChanged` fires for whichever values the new world changed.
