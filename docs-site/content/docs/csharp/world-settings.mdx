...
| `OnSelectedChanged` | yes | it fires while the host is picking values on the world settings screen, but **not** while Gears loads a world's values in |
...
a world path. Gears binds it **invoke-only**: the method is never raised by a change, and is instead
called with the setting's current `SettingValue` every time you call `SyncSettingsToClass`.
That is the natural shape for a world value, which only ever arrives with a world. Because that call
is its only way to run, it has to be written with `includeInSync: true`.
...
Loading those values raises no `OnSelectedChanged`, on any of the three routes above — that callback
is how a mod finds out, and `SyncSettingsToClass` is how it gets the values. See [Loading a
value in raises nothing](/docs/csharp/setting-listeners#loading-a-value-in-raises-nothing).
...
class](/docs/csharp/setting-listeners#sync-settings-to-a-class).
