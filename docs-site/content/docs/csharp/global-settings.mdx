...
Create settings in `InitMod`, before Gears reads `ModSettings.xml`:
...
Gears merges settings you create in `InitMod` with whatever `ModSettings.xml` declares under the
same names, then restores the player's saved value on top.
